...
詳しくは@<chapref>{91-compare}を参照してください。
//}

本書では@<em>{GetFEM}と@<em>{PyVista}というライブラリを使用します。

 * @<em>{GetFEM}とは有限要素法コードを実装するためのフレームワークです。
   有限要素法のモデルを解くのに必要です。
 * @<em>{PyVista}とは3次元可視化フレームワークです。
   有限要素法の解析結果を可視化するのに必要です。
