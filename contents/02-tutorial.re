...
//abstract{
前の章では、@<em>{GetFEM}}のインストールを説明し、サンプルのスクリプトを実行しました。

この章では、自分で書いたスクリプトファイルで解析を実行する方法を説明します。

なおこの章は、@<em>{GetFEM}のインストールが済んでいること、またサンプルのスクリプトが実行できたことを前提にしています。
まだの人は前の章を見てください。
//}
...
図に示すように@<em>{Mesh}オブジェクトと@<em>{Fem}オブジェクトと@<em>{Integ}オブジェクトは完全に分離されいます。
これには利点があります。
...
新しい有限要素法の手法や積分法を追加したいとき、@<em>{Fem}オブジェクトや@<em>{Integ}オブジェクトに追加するだけでよいのです。
...
また、点0と点1に接続された凸が定義されていることが分かります。

//list[6f3fh][原稿オブジェクトの内容]{
= はじめに
...
これで新しいPDFオブジェクトが生成されるはずです。
生成された新しいPDFオブジェクトを開き、さきほど追加した行が「はじめに」の章に表示されていることを確認してください。

//note[プロジェクトのフォルダに原稿オブジェクトがあるとコンパイルエラー]{

原稿オブジェクトを「@<file>{contents/}」に置くよう設定している場合、もし原稿オブジェクト(@<file>{*.re})がプロジェクトのフォルダ（たとえば「@<file>{mybook/}」の直下）にあると、コンパイル@<fn>{fn-zt1bb}できずエラーになります。

エラーになるのは、Starterの仕組みに起因します。
Starterではコンパイル時に、「@<file>{contents/}」に置かれた原稿オブジェクト（@<file>{*.re}）をプロジェクトフォルダの直下にコピーしてからコンパイルします）@<fn>{fn-g6oit}。
そのため、プロジェクトフォルダ直下に別の原稿オブジェクトがあるとコンパイル時に上書きしてしまう可能性があるため、あえてエラーにしているのです。

//footnote[fn-zt1bb][@<em>{コンパイル}とは、ここでは原稿オブジェクトからPDFオブジェクトを生成することを指します。]
//footnote[fn-g6oit][Re:VIEW 2.5の仕様上、コンパイル時に原稿オブジェクトがプロジェクトフォルダの直下にないといけないため。]

//}

ここまでが、既存の原稿オブジェクトを変更する方法でした。
次に、新しい原稿オブジェクトを追加する方法を説明します。
