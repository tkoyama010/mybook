...
//abstract{
前の章では、@<em>{GetFEM}のインストールを説明し、サンプルファイルを実行しました。

この章では、自分で@<em>{GetFEM}実行する方法を説明します。

なおこの章は、@<em>{GetFEM}のインストールが済んでいること、またサンプルファイルの実行ができたことを前提にしています。
まだの人は前の章を見てください。
//}
...
慣例として@<em>{GetFEM}はgfとしてインポートされるので覚えておきましょう。
